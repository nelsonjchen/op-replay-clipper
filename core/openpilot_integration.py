...
def _patch_ui_application_record_skip(path: Path) -> bool:
    ...
    record_gop_line = 'RECORD_GOP_FRAMES = os.getenv("RECORD_GOP_FRAMES", "")  # Optional ffmpeg GOP size in frames\n'
    record_force_keyframes_line = 'RECORD_FORCE_KEYFRAMES = os.getenv("RECORD_FORCE_KEYFRAMES", "")  # Optional ffmpeg keyframe timestamps\n'
    ...
    if record_codec_line not in updated and record_skip_line in updated:
        updated = updated.replace(
            record_skip_line,
            record_skip_line + record_codec_line + record_preset_line + record_tag_line + record_gop_line + record_force_keyframes_line,
            1,
        )
    ...
    if "RECORD_GOP_FRAMES" not in updated:
        upgraded_gop, count = re.subn(
            r"""        if RECORD_TAG:\n"""
            r"""          ffmpeg_args \+= \['-tag:v', RECORD_TAG\]\n""",
            """        if RECORD_TAG:\n"""
            """          ffmpeg_args += ['-tag:v', RECORD_TAG]\n"""
        ...
        if count:
            updated = upgraded_gop
    if "RECORD_FORCE_KEYFRAMES" not in updated:
        upgraded_force_keyframes, count = re.subn(
            r"""        if RECORD_TAG:\n"""
            r"""          ffmpeg_args \+= \['-tag:v', RECORD_TAG\]\n""",
            """        if RECORD_TAG:\n"""
            """          ffmpeg_args += ['-tag:v', RECORD_TAG]\n"""
        ...
        if count:
            updated = upgraded_force_keyframes
